...
func main() {
	...
	var (
		...
		serviceName    = fs.String("service-name", "authn-cli", "")
		//caCerts        = fs.String("ca-certs", "/Users/cage/qnap/quai/gokitconsul/deployments/docker/ssl/localhost+3.pem", "tls based credential")
		caCerts        = fs.String("ca-certs", "", "tls based credential")
		httpAddr       = fs.String("http-addr", "", "HTTP address of authn")
		grpcAddr       = fs.String("grpc-addr", "", "gRPC address of authn")
		...
		method         = fs.String("method", "login", "login, add")
	)
	...
	if *httpAddr != "" {
		svc, err = authntransports.NewHTTPClient(*httpAddr, tracer, zipkinTracer, log.NewNopLogger())
	} else if *grpcAddr != "" {
		...
		if *consulHost != "" && *consultPort != 0 {
			conn, err = grpc.Dial(
				...
				grpc.WithBalancer(grpc.RoundRobin(grpclb.NewConsulResolver(
					fmt.Sprintf("%v:%d", *consulHost, *consultPort), "grpc.health.v1.authn",
				))),
			)
			if err != nil {
				level.Error(logger).Log("serviceName", "grpc.health.v1.authn", "error", err)
				os.Exit(1)
			}
		} else {
			if *caCerts != "" {
		...
		svc = authntransports.NewGRPCClient(conn, tracer, zipkinTracer, logger)
	}
	...
	case "login":
		email := fs.Args()[0]
		password := fs.Args()[1]
		v, err := svc.Login(context.Background(), model.User{
			email, password,
		})
		if err != nil {
	...
	case "add":
		email := fs.Args()[0]
		password := fs.Args()[1]
		err := svc.Add(context.Background(), model.User{
			email, password,
		})
		if err != nil {
			st, ok := status.FromError(err)
		...
		fmt.Fprintf(os.Stdout, "add %v, %v", email, password, )
	default:
